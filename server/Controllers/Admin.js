...
const serviceSchema = require("../Models/service_schema");
...
const InsertService = async (req, res) => {
  try {
    ...
    console.log("Service info inserted successfully");
    res.json({
      success: true,
      message: "Insertion Successful",
      newService: savedService,
    });
  } catch (err) {
    console.log("Error occured" + err);
    res.status(400).json({ error: "Invalid Data" });
...
const GetSingleService = async (req, res) => {
  try {
    let service = await  serviceSchema.find(req.params.id);
    console.log("Service info fetched from the database");
...
const updateService = async (req, res) => {
  try {
    ...
    if (!service) {
      ...
      if (picture) {
        updatedService.picture = picture;
      }
      ...
      console.log("Service information updated successfully");
      res.json({
        success: true,
